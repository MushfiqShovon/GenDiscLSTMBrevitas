...
cleaned_train_file = load_and_preprocess('./train.data')
cleaned_valid_file = load_and_preprocess('./valid.data')
cleaned_test_file = load_and_preprocess('./test.data')
...
class LSTMNet(nn.Module):
    ...
    def __init__(self,vocab_size,embedding_dim,hidden_dim,output_dim,n_layers,bidirectional,dropout):
        ...
        self.lstm = nn.LSTM(embedding_dim, hidden_dim, num_layers=n_layers, bidirectional=bidirectional, dropout=dropout, batch_first=True)
        self.quantLSTM = qnn.QuantLSTM(input_size=100, hidden_size=hidden_dim, num_layers=n_layers, bidirectional=bidirectional, weight_bit_width=4)

        
        self.fc = qnn.QuantLinear(hidden_dim, output_dim, bias=False, weight_bit_width=8)  # Adjusted from hidden_dim * 2 to hidden_dim
    ...
    def forward(self,text,text_lengths):
        ...
        #print("Generating QuantLSTM Layer")
        packed_output,(hidden_state,cell_state) = self.lstm(packed_embedded)
        # Concatenating the final forward and backward hidden states
        #hidden = torch.cat((hidden_state[-2,:,:], hidden_state[-1,:,:]), dim = 1)
        #print("QuantLSTM Layer Generated")
